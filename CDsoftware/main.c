...
extern uint32_t pulsouts[8];//={0, 0,  1<<2, 1<<15, 1<<4, 1<<12, 1<<3, 1<<11};

extern volatile uint16_t *pulsoutHI[8];//={&(GPIOB->BSRRL), &(GPIOB->BSRRL), &(GPIOB->BSRRL), &(GPIOC->BSRRL), &(GPIOB->BSRRL), &(GPIOA->BSRRL), &(GPIOB->BSRRL), &(GPIOA->BSRRL) };
//                                  0              0              PB2                PC15           PB4               PA12           PB3                PA11 
extern volatile uint16_t *pulsoutLO[8];//={&(GPIOB->BSRRH), &(GPIOB->BSRRH), &(GPIOB->BSRRH), &(GPIOC->BSRRH), &(GPIOB->BSRRH), &(GPIOA->BSRRH), &(GPIOB->BSRRH), &(GPIOA->BSRRH) }; // both are 16 bit registers

extern volatile uint32_t intflag[4];//={0,0,0,0}; // interrupt flag...
...
int main(void)
{
  ...
  */    
  ...
  TIM_TimeBase_InitStructure.TIM_Prescaler = 220; // was 8//16 before// but check it what speed is this 18khz toggle = 36k  - how we can check - with one of our pins as out // was 21 (now 32 for latest geomantic code // 220 for 4x HH
  ...
  // testings
  //      DAC_SetChannel1Data(DAC_Align_12b_R, 1204); // 1000/4096 * 3V3 == 0V8
  //  ADC_RegularChannelConfig(ADC1, ADC_Channel_13, 1, ADC_SampleTime_144Cycles); 
  uint32_t tmp, x, cnt=1;
  ...
  if (intflag[1]) { // process INT - 1 is left hand clock!
    intflag[1]=0;
    tmp=2;
    *pulsoutLO[tmp]=pulsouts[tmp];
    for (x=0;x<cnt;x++){
    delay();
    }
    *pulsoutHI[tmp]=pulsouts[tmp];
  }
      */
